...
Go 实现的垃圾回收器是无分代（对象没有代际之分）、 不整理（回收过程中不对对象进行移动与整理）、并发（与用户代码并发执行）的三色标记清扫算法。本文分析Go触发gc的3种方式。
...
当用户调用 runtime.GC 方法后，判断上一轮GC是否已经完成，校验通过就会触发主动gc

- 分配对象时触发

当p.mcache已用尽或者分配大于32kb的大对象时，就会尝试触发gc，校验通过后就会触发gc

- 定时触发

由 forcegchelper 守护协程每2分钟触发一次gc